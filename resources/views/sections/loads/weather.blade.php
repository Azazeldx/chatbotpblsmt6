    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:!px-20 py-12">
        <div class="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div class="bg-gradient-to-br from-gray-700 to-gray-900 rounded-2xl overflow-hidden h-80 relative shadow-2xl group cursor-default" 
                ...
                <img src="{{ asset('storage/weather/' . ($data['weather']['custom_bg'] ?? 'PnbGoodWeather.webp')) }}" 
                ...
                <div class="absolute inset-0 flex flex-col justify-center items-center text-white transition-all duration-500 group-hover:scale-105">
                    <div class="text-7xl font-bold mb-1 drop-shadow-lg">
                        {{ $data['weather']['main']['temp_floor'] ?? 'N/A' }}°C
                    </div>
                    <div class="text-xl font-medium tracking-widest uppercase opacity-90 border-t border-white/30 pt-2 drop-shadow-md">
            ...
            <div data-aos="fade-left" data-aos-duration="1000" data-aos-delay="200">
                <h3 class="text-blue-600 font-bold tracking-widest text-sm mb-2 uppercase">KONDISI KAMPUS PNB</h3>
                ...
                <div class="flex gap-6 mb-6 text-gray-700 border-y border-gray-200 py-4">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm">Angin: <strong>{{ $data['weather']['wind']['speed'] ?? 0 }} km/h</strong></span>
                    </div>
                    <div class="flex items-center gap-2">
                        <i class="fas fa-tint text-blue-400"></i>
                        <span class="text-sm">Lembap: <strong>{{ $data['weather']['main']['humidity'] ?? 0 }}%</strong></span>
                    </div>
                    <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-4 mb-6">
                    <div class="bg-blue-600 p-3 rounded-xl shadow-lg shadow-blue-200">
                        <i class="fas {{ ($data['weather']['weather'][0]['main'] ?? '') == 'Rain' ? 'fa-cloud-showers-heavy' : 'fa-cloud-sun' }} text-white text-2xl"></i>
                    </div>
                    <span class="text-gray-800 font-bold text-xl tracking-tight">
                        {{ $data['weather']['name'] ?? 'Jimbaran' }}, Bali
                    </span>
                </div>

                <div class="bg-white p-6 rounded-xl border-l-8 {{ ($data['weather']['weather'][0]['main'] ?? '') == 'Rain' || ($data['weather']['weather'][0]['main'] ?? '') == 'Thunderstorm' ? 'border-red-500' : 'border-blue-600' }} shadow-md">
                    <p class="text-gray-600 leading-relaxed italic text-lg">
                        "{{ $data['weather']['pesan_himbauan'] ?? 'Selamat beraktivitas di Politeknik Negeri Bali.' }}"
                    </p>
                </div>
            </div>
