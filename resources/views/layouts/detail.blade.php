            ...
            <div class="w-full lg:w-[70%]">
                <div class="mb-8">
                    ...
                    <div class="flex items-center gap-4 mb-8 text-sm text-gray-600">
                        ...
                        <div class="flex flex-col">
                            <p class="font-bold text-gray-900 text-base leading-none mb-1">{{ $data['article']->creator->name }}</p>
                            <div class="flex items-center gap-2 text-xs text-gray-400 font-medium">
                                ...
                                <span class="hidden md:inline text-[#3b4279]/60 bg-[#3b4279]/5 px-2 py-0.5 rounded ml-1">
                ...
                <div class="mt-12 pt-8">
                    <div class="relative mb-8 pb-4">
                        ...
                        <div class="absolute bottom-0 left-0 w-24 h-[3px] bg-[#3b4279] z-10"></div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-4">
                        @foreach ($data['article']->tags as $tag)
                            <div class="transform transition-all duration-300 hover:-translate-y-1 hover:scale-105">
                                <x-links.tag 
                                    search="{{ $data['navigation']['search'] ? true : false }}" 
                                    category="{{ $data['article']->category->slug }}" 
                                    tag='{{ $tag->tag_name }}' 
                                    slug='{{ $tag->slug }}'
                                    class="!inline-flex !items-center !px-8 !py-4 !text-base lg:!text-lg !font-black !uppercase !tracking-widest !rounded-2xl !bg-gray-100 !text-gray-800 hover:!bg-[#3b4279] hover:!text-white shadow-md hover:shadow-xl border border-gray-200/50"
                                />
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="w-full lg:w-[30%]">
                <div class="sticky top-24">
                    <div class="relative mb-8 pb-4">
                        ...
                        <div class="absolute bottom-0 left-0 w-32 h-[3px] bg-[#3b4279] z-10"></div>
                    </div>
                    @if (!$data['related']->isEmpty())
                        <div class="flex flex-col gap-6">
                            @foreach($data['related'] as $related)
                                    ...
                                    <a href="{{ route('detail', ['category' => $related->category->slug, 'slug' => $related->slug]) }}" class="group flex gap-4 transition-all duration-300 hover:bg-gray-50 p-2 rounded-xl">
                                        ...
                                        <div class="flex flex-col justify-center">
                                            <h4 class="text-sm font-bold leading-tight text-gray-800 group-hover:text-[#3b4279] transition-colors line-clamp-2">
                                                {{ $related->title }}
                                            </h4>
                                            <div class="flex items-center gap-2 mt-2">
                                                <span class="text-[10px] font-bold text-[#3b4279] bg-[#3b4279]/10 px-2 py-0.5 rounded uppercase">
                    ...
                    @else
                        <p class="text-gray-400 text-sm italic">Belum Ada {{ $data['article']->category->category_name }} Terkait.</p>
                    @endif

                    {{-- CTA Box --}}
                    <div class="mt-10 p-6 bg-[#3b4279]/5 rounded-2xl border border-[#3b4279]/10">
                        <h5 class="font-bold text-[#3b4279] mb-2">Fasilitas Politeknik Negeri Bali</h5>
                        <p class="text-xs text-gray-600">Jelajahi fasilitas kampus kami melalui Realitas Virtual 360°.</p>
                        <a href="/virtual/index.htm" target="_blank" class="inline-block mt-4 text-xs font-bold text-[#3b4279] hover:underline">MULAI TUR &rarr;</a>
                    </div>
                </div>
            </div>
        </div>
